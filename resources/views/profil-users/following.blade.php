...
<div class="m-auto" style="width: 84%">
    <div class="row">
        ...
        <div class="col-md-8 row mt-5 d-flex">
        
                    @forelse ($listfollowing as $following)
                    <div class="bg-white border  mt-2 col-4" style="margin-left: 1px;height: 150px;  background: url('/img/{{ $following->avatar }}') no-repeat;  background-size: cover;  border-radius: 22px #665A5C;box-shadow: 4px 2px 4px #665A5C;">
                        <div class="row">
                                <a href="{{ url('/' . $following->username) }}" class="text-dark col-6">
                                    <h4 class="list-group-item-heading">{{ $following->name }}</h4>
                                    <strong class="list-group-item-text">@ {{ $following->username }}</strong>
                                
                                </a>
                                <strong class="list-group-item-text text-dark col-6">{{ $following->created_at->diffForHumans() }}</strong>
                            </div>
                    </div>
                    @empty
